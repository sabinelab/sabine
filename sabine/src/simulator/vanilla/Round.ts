...
export default class Round extends Match {
  public override async start() {
    ...
    for (const t of this.teams) {
      ...
      for (const p of t.roster) {
        ...
        if (p.weapon.primary) {
          const weapon = valorantWeapons.filter(
  ...
  private async finish(score1: number, score2: number) {
    ...
    if (this.mode === "ranked") {
      ...
      (this.mode === "tournament" && !this.overtime)
    ) {
      await this.saveMatch({
        winnerIdx,
        score1,
  ...
  private async saveMatch(options: SaveMatchOptions) {
    ...
    if (winKey && loseKey) {
      ...
      if (updateRank)
        loseData.rankRating = p1Win ? profile2.rankRating : profile1.rankRating;
  ...
  private async firstStep(duels: number) {
    ...
    if (!playersAlive1 || !playersAlive2) {
      ...
      if (bombPlanted) {
        ...
        const bombSite =
          bombSites[Math.floor(Math.random() * bombSites.length)];
