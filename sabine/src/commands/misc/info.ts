...
export default createCommand({
  ...
  async run({ ctx, app, t }) {
    const [creator, guildsCount, usersCount] = await Promise.all([
      ...
      app.shard?.fetchClientValues("guilds.cache.size") as Promise<
        number[] | undefined
      >,
      app.shard?.broadcastEval(
        (a) => a.users.cache.filter((u) => !u.bot).size
      ) as Promise<number[] | undefined>
    ]);

    const totalUsers = usersCount?.reduce((sum, count) => sum + count, 0) ?? 0;
    const totalGuilds =
      guildsCount?.reduce((sum, count) => sum + count, 0) ?? 0;
